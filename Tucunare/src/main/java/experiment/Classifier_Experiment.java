...
public class Classifier_Experiment {
	//Variaveis estaticas
...
//					Imprime resultados para cada modelo(treino e teste)
					//System.out.println(projects[0].toString()+","+(w-1)+","+count+","+(trainSizeInicio)+","+retornaClassificador(cls[j]).getClass().getSimpleName()+","+String.valueOf(new DecimalFormat("0.##").format((double)acc1.get(0)).replace(',', '.'))+","+eval.kappa());
...
//				double sd = Math.sqrt(variancia.evaluate(acc)); 
				...
				for(int c=1; c<=1; c++){
					...
					System.out.println(projects[0].toString()+","+retornaClassificador(cls[j]).getClass().getSimpleName()+kernel+","+c+","+String.valueOf(new DecimalFormat("0.##").format((double)media[c-1]).replace(',', '.'))+","+String.valueOf(new DecimalFormat("0.##").format((double)(accMatriz)).replace(',', '.')));
				}
...
//	Métodos estáticos
	...
	public static int [] retornaClassificadores(){
		int [] classificadores = {1};//1->"J48",2->"NaiveBayes",3->"RandomForest",4->"Ibk",5->"SMO(Poly)",6->"SMO(Puk), 7->"SMO(RBF)"
	...
	public static Classifier retornaClassificador(int classificador){
		...
		case 3: 
			...
			rf.setNumIterations(100); rf.setMaxDepth(0); rf.setNumExecutionSlots(4); rf.setSeed(1);//rf.setNumFeatures(0);
			return rf;
		case 4: 
			IBk ibk = new IBk(19);//fazer as variações ímpares de k
			return ibk;	
		case 1: 
			return new J48();	
		default:
