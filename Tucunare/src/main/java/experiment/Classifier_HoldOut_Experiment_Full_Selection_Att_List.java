...
public class Classifier_HoldOut_Experiment_Full_Selection_Att_List {
	//Variaveis est�ticas
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println("Project,Id,Attibute,Rank");
		...
		for(int a=0;a<listaArquivos.length;a++){ //la�o para percorrer todos os arquivos do vetor
			...
			for(int w=10;w<=limitTest;w++){
				for(int tr=10; tr<=10; tr++){
					...
					CorrelationAttributeEval eva = new CorrelationAttributeEval();
					attSelectionPos.setEvaluator(eva);
					attSelectionPos.setRanking(true);
					Ranker rank = new Ranker();
					rank.setNumToSelect(position);
					rank.setThreshold(-1.7976931348623157E308);
					attSelectionPos.setSearch(rank);
					attSelectionPos.SelectAttributes(train);

					int [] attributes = attSelectionPos.selectedAttributes();
					double [][] ranked = attSelectionPos.rankedAttributes();
