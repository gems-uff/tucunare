...
public class SaveFile implements Runnable {
	...
	public void run() {
		...
		try {
			...
			System.err.println("Erro ao processar os dados do repositÃ³rios "+repo);
			e.printStackTrace();
		}
	}

	//Usar threads para recuperar os dados e salvar tudo o que foi armazenado na String em uma Ãºnica escrita de arquivo.
	public String retrieveData(String repo, Settings settings) throws UnknownHostException {
		...
		try{
			 ...
			 */
			Class<SaveFile> cls = SaveFile.class;

			List<Method> meth = new ArrayList<Method>();
			try {
				for (String aux: settings.getMethods())
					meth.add(cls.getMethod(aux));//recuperando os métodos.
			}catch(Exception e){
				System.err.println("Método "+e.getMessage()+" nÃ£o encontrado.");
			}
			...
			for (DBObject dbObject : cursor) {
				...
				if(dbObject!=null){// & !closed_by.equals(user)){
					...
					//tratamento para caminho dos arquivos para buscar o Ãºltimo diretÃ³rio
					String dirFinal = "";
					...
					//Desenvolvedor que integrou ou rejeitou o cÃ³digo do pull request
					...
					if (!saveFile(false, resultTemp))
						System.err.println("Erro ao tentar escrever o PR: "+(Integer) dbObject.get("number")+", do repositÃ³rio: "+repo);
				}else
					continue;
	...
	public String getAllRepoData(){
		return "All repo data";
	}

	public String getContributors(){
		return "Contributors x,y and z";
	}
