...
public class SaveFileAuthor implements Runnable {
	private String repo = ""; 
	private String file;
	...
	public SaveFileAuthor(String repo, String file, Settings settings) throws UnknownHostException{
		this.repo = repo; 
		this.file = file;
...
//							files.replace(", ", "|");
	...
	public boolean saveFile(boolean firstWritten, String pullRequestData){
		File fileTemp = new File(file+File.separator+repo+"_authorMoreCommits.csv");
		FileWriter fw = null;
		try {
			if (firstWritten){
				fw = new FileWriter(fileTemp);
				fw.write("numberPull,authorMoreCommits");
						//"owner/repo,ageRepoDays,stargazersCount,watchersCount,language,forksCount,openIssuesCount,subscribersCount,has_wiki,contributors,acceptanceRepo,"
