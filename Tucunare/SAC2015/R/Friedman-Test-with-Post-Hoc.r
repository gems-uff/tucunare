	...
	if(to.post.hoc.if.signif)
		{
          ...
          #library(ggplot2)
					...
					#update_labels(p, list(x = "Classificadores", y = "Diferenças"))+labs(title = "Boxplots (of the differences)") + theme(axis.text = element_text(size = "14", colour = "black"))+ theme(plot.title = element_text(size = rel(2)))+ theme(axis.title.y = element_text(size = rel(1.5), angle = 90))+ theme(axis.title.x = element_text(size = rel(1.5)))
					
					boxplot(Y.b.minus.a.combos,
						names = names.b.minus.a.combos ,
						col = is.signif.color,
						main = "Boxplots das diferenças",
						ylim = the.ylim,
						las = 2,
						cex.axis = 0.6
						)
